...
Este código asume que ya cuentas con la matriz de cuentas en la variable cts, y una tabla
con la información de las muestras en coldata. El diseño indica cómo modelar las muestras,
en este caso se quiere medir el efecto de la condición, controlando las diferencias batch
Las dos variables batch y condition deben ser columnas en coldata.
...
Importante: Debes incorporar cuentas no normalizadas, o cuentas estimadas de lecturas de secuenciación
DESEq2 internamente corrige los tamaños de librerías
...
Este es un experimento realizado en cultivos de Drosophila melanogaster, investigaron el efecto
del knockdown de Pasilla, los datos se pueden obtener en GEO: GSM461176 a GSM461181
...
Ya que tenemos los datos, ahora si podemos construir el objeto DESeqDataSet
...
Aunque no es necesario pre-filatar los genes con conteos bajos cuando corremos las funciones de DESeq2, hay dos razones por las cuales vale la pena hacerlo, si eliminamos las filas en las que no hay muchos reads, 
...
si no le dices a DESeq2, que nivel quieres comparar contra qué 
...
El modelo DESeq2 está descrito en detalle en la publicación (Love, Huber, and Anders 2014),
realiza una normalización interna, se calcula una media geométrica para cada gen a través de todas las muestras. Las cuentas para cada gen en cada muestra se dividen por esta media.
La mediana de estas tasas en una muestra es el size factor para esa muestra. Este procedimiento corrige el tamaño de las librerías y el bias de la composición de RNA, que puede surgir por ejemplo cuando un número pequeño de genes se expresa muy alto en una condición experimental pero no en otra. Cuando se tiene un número pequeño de réplicas es difícil estimar la varianza intra-grupo con confianaza, DESeq2 usa una estimación shrinkage para dispersiones y fold changes. Un valor de dispersión se estima para cada gen con un modelo de ajuste. Necesitas un número suficiente de réplicas biológicas para estimar la dispersión de forma apropiada. Si no hay, entonces DESeq la estima usando muestras de las diferentes condiciones como si fueran réplicas. DESeq ajusta modelos lineales generalizados de binomial negativa para cada gen y usa la prueba Wald para significancia estadística. Además de la información de grupo es posible que ustedes puedan darle información adicional para pareados en el análisis.
...
# No te olvides de correr esto en la terminal y no en el Rmarkdown!
